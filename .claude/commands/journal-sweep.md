...
- **Open-access full text (PMC, DOAJ-indexed, or the journal's own open-access articles)**: fetch and read the real full text (WebFetch/defuddle), critique properly - actual sample size, actual methodology limitations, whether the result generalizes to the claim being evaluated. This is the same rigor `/concept-audit`'s study-verification step already applies.
