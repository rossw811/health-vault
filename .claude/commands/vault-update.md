...
Check `Get-ScheduledTask -TaskName "HealthVault-YouTubeQueue-Loop"` (PowerShell). If its `State` isn't `Running`, restart it (`Start-ScheduledTask`). **Partially fixed 2026-08-08** (see `buglog.md`): the task now has `RestartCount`/`RestartInterval`/`StartWhenAvailable` configured, so it recovers on its own from a crash or a missed trigger while the machine was asleep. It still does **not** survive a reboot with nobody logged in — that requires switching its logon type off `Interactive`, which needs elevated rights Claude doesn't have; report the state either way in the summary, and if it's down after a reboot specifically, that's the one case still requiring a manual restart.
