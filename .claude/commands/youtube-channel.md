...
4.5. **Signal-density flag** (added 2026-08-01, for on-topic videos that pass step 4.4 — this is a graduated quality signal, not a second relevance gate). Some channels are consistently relevant-but-thin (e.g. a long interview show where health/fitness only comes up briefly amid a lot of unrelated conversation) rather than uniformly on-topic or uniformly junk — a binary relevance check alone can't distinguish "every video is dense with real content" from "every video technically qualifies but is 90% filler." After extraction (step 7.5), judge the ratio of substantive, citable content to total relevant-topic runtime and set `signal_density: high | mixed | low` in the video note's frontmatter:
   - **high**: most of the on-topic portion is substantive (real claims, mechanisms, citable material) — this is the default for most channels this vault already tracks closely.
   - **mixed**: real substance exists but is diluted by a lot of tangential/repetitive/low-effort content even within the on-topic portion — typical of long-form interview shows covering many subjects per episode where health is one thread among several.
   - **low**: technically on-topic (passes step 4.4) but thin — a passing mention stretched into a long video, or content that's mostly restating well-covered material with nothing new.
   This is separate from the promo/sponsor-segment exclusion in step 7.5 (which strips ad reads regardless of the surrounding content's density) and separate from step 4's discard (an off-topic video never reaches this flag at all).
...
- Frontmatter: `type: channel-rollup`, `channel`, `channel-url`, `video-count-total`, `video-count-new-this-run`, `video-count-relevant`, `video-count-discarded-irrelevant`, `signal-density-high`, `signal-density-mixed`, `signal-density-low` (counts among this run's relevant videos, per step 4.5), `date`, `tags`.
...
- **Content signal**: does this channel consistently produce substantive, on-topic material, or was it mostly filler/promo even among the "relevant" videos? State the signal-density breakdown from step 4.5 (e.g. "12 high / 8 mixed / 3 low this run") rather than just a prose impression — this is exactly the distinction that separates "relevant but thin" channels (high relevance hit-rate, mostly `mixed`/`low` signal density — typical of a long-form interview show where health is one thread among several) from genuinely dense ones, without needing to exclude the channel outright.
- **Recommendation tier**: `follow closely` (consistently valuable — high relevance hit-rate AND mostly `high` signal density), `follow loosely` (occasionally useful — either a lower relevance hit-rate or a `mixed`-heavy signal-density profile), `deprioritize` (mostly irrelevant or thin — `low`-heavy signal density even when technically on-topic, or a low relevance hit-rate; technically not excluded, not worth actively re-checking often).
