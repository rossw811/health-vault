---
description: Ingest every video from a YouTube channel into the vault (resumable, permanently excludes inactive/sparse channels, skips ads/sponsor segments), then link concepts across the run and fill gaps via /research. Add --limit N to cap a run, --visual for frame reading.
category: research
---
...
## 11. Summary to user
Total videos on channel, new this run, failed, concepts linked, gaps filled, rollup note path. If this is a re-run, explicitly say "N new videos since last run on <date>". If excluded at step 3, this is the whole report.
