...
**Deliberately NOT scheduled** — `/process-raw-transcripts` (see above — costs real budget per run, must be invoked as a single batched agent call, not fired blindly), `/oura-analyze`, `/concept-audit` — re-run manually as history grows.

Manage/inspect all scheduled tasks via Task Scheduler (`taskschd.msc`). Two durable lessons learned building this stack, worth remembering before adding anything new to it: (1) `Stop-ScheduledTask` does not reliably kill an entire child process tree, and force-killing a parent doesn't clean up a `ProcessPoolExecutor`'s child workers — they can orphan and hold loaded models in memory indefinitely; any new long-running background script should use a PID-based singleton lock like the two collectors do. (2) A scheduled task's `Execute` field needs the interpreter's full path (e.g. `C:\Users\RossW\anaconda3\python.exe`), never a bare `python.exe`/`powershell.exe` — PATH does not reliably resolve in the actual unattended execution context, the same lesson learned earlier for `ffmpeg`.
